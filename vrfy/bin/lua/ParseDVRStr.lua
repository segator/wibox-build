...
local function DataProcess(str)
	...
	elseif cmd == receivedata[2] then
		...
		for systemtime in string.gfind(str, "|(.+)(|?)") do
			...
			result  = COM.ExtraProcess(Lua2C.emendtime, channel, systemtime);	
		end;		
	...
	elseif cmd ==  receivedata[3] then
		...
		if number then
			...
			result = result and COM.AppendCard(Lua2C.cardno, channel, number);
		end;
		
		if id then
			result = result and COM.AppendCard(Lua2C.machineno, channel, id);	
		end;
		
		if time then
			result = result and COM.AppendCard(Lua2C.transaction_time, channel, time);
		end;	
